...
def loadModuleFile(moduleName, filePath):
    """Load a Python module from a file using modern importlib"""
    spec = importlib.util.spec_from_file_location(moduleName, filePath)
    if spec is None:
        raise ImportError(f"Could not load spec for module '{moduleName}' from {filePath}")
    ...
    try:
        ...
        raise ImportError(f"Error loading module '{moduleName}': {e}")
...
def evaluate(generateSolutions, testRoot, moduleDict, exceptionMap=ERROR_HINT_MAP,
    ...
    for q in test_subdirs:
        ...
        tests = [t for t in os.listdir(subdir_path) 
                if re.match(r'[^#~.].*\.test$', t)]
        tests = [re.match(r'(.*)\.test$', t).group(1) for t in tests]
...
if __name__ == '__main__':
    ...
    moduleDict['projectTestClasses'] = loadModuleFile(
        moduleName, 
        os.path.join(options.codeRoot, options.testCaseCode)
    )
